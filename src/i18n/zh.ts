...
export const zh: Translations = {
  ...
  settings: {
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Meridian 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Meridian 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Meridian 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Meridian 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Meridian Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Hermes 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Meridian Desktop 将使用已保存设置重新连接。',
  ...
  updates: {
    ...
    applyingBody: 'Meridian 更新器会在自己的窗口中接管，并在完成后重新打开 Hermes。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Hermes 会自动重新连接。',
    applyingClose: 'Meridian 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  onboarding: {
    ...
    preparingInstall: 'Meridian 正在完成安装。首次运行通常不到一分钟。',
  ...
  preview: {
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Meridian 正在重启...',
      askRestart: '让 Hermes 重启服务器',
      lookingRestart: taskId => `Meridian 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Meridian 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `Meridian 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Meridian 无法重启服务器。',
      stillWorking: 'Meridian 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Meridian 正在加载回复',
      thinking: '思考中',
      today: time => `今天，${time}`,
    ...
    approval: {
      gatewayDisconnected: 'Meridian 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      moreOptions: '更多审批选项',
      allowSession: '允许本会话',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Meridian 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Meridian 需要一个凭据才能继续。',
