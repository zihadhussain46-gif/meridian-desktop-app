    ...
    <PreviewEmptyState
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Meridian is restarting...' : 'Ask Hermes to restart the server',
  ...
  useEffect(() => {
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'Meridian could not restart the server.',
        durationMs: 6000
      })
    }
