...
let bootProgressState = {
  ...
  message: 'Waiting to start Meridian backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Meridian updater…', percent: 100 })
...
async function applyUpdatesPosixInApp() {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Meridian (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Meridian to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Meridian.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Meridian.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Meridian to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Meridian to load the new version.',
      percent: 100
    })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Meridian.app/Contents/...`
...
function resolveHermesBackend(dashboardArgs) {
  ...
  return {
    ...
    label: 'Meridian Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Meridian install was cancelled.')
  ...
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Meridian runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Meridian gateway',
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Meridian backend', 8)
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Meridian runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(dashboardArgs))
    const hermesCwd = resolveHermesCwd()
    const webDist = resolveWebDist()
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Meridian backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Meridian backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Meridian backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Meridian backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Meridian backend to become ready', 90)
    await Promise.race([waitForHermes(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Meridian backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
